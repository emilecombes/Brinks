...
public class Scheduler {
    ...
    public List<Route> scheduleRoutesForDepot(Depot depot, Set<Customer> customers, Day day) {
        ...
        System.out.println("Model.Day  -----------" + day.getId());
        ...
        while (!customers.isEmpty()) {
            ...
            //TODO isFesibleWith
            if (isFeasibleWith(newRoute, newCustomer, day.getId())) {

                newRoute.getVisits().add(newRoute.getVisits().size() - 1, newCustomer);
                updateRoute(newRoute, day.getId());
            } else {
    ...
    public boolean isFeasibleWith(Route route, Customer customer, int dayId) {
        ...
        //ADD ROUTE AND CALCULATE NEW VALUES
        ...
        for (int i = 1; i < EarliestDepartureTimes.length; i++) {
            if (EarliestDepartureTimes[i - 1] + durations[i] + input.times.get(route.getVisits().get(i - 1).getId()).get(route.getVisits().get(i).getId()) > LatestStartTimes[i]) {
                feasible = false;
            }
        }
    ...
    public float calculateCostPerRoute(Route route) {
        ...
        float serviceCost = duration * input.cost_wage_minute;
        int distance = 0;
